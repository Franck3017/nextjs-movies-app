...
export async function GET(
  ...
  try {
    ...
    const url = `https://api.themoviedb.org/3/discover/movie?api_key=${TMDB_API_KEY}&language=es-ES&with_genres=${params.id}&page=${page}&sort_by=${tmdbSortBy}&include_adult=false&include_video=false`
    ...
    return NextResponse.json(
      { error: 'Error fetching genre movies' },
      { status: 500 }
    )
  }
} 